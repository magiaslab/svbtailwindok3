  ...
  if (!standings || standings.length === 0) {
    return (
      <div className={`text-center py-8 ${className}`}>
        <p className="text-gray-500 dark:text-gray-400">Nessun dato disponibile per i grafici</p>
      </div>
    );
  }
  ...
  return (
    <div className={`space-y-8 ${className}`}>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6">
        <div ref={el => chartRefs.current.pointsChart = el} />
      </div>

      {/* Grafico Vittorie vs Sconfitte */}
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6">
        <div ref={el => chartRefs.current.winsLossesChart = el} />
      </div>

      {/* Grafico Differenza Punti */}
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6">
        <div ref={el => chartRefs.current.pointsDiffChart = el} />
      </div>

      {/* Grafico Percentuale SVB */}
      {standings.find(team => team.team === 'Basket San Vincenzo') && (
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6">
          <div ref={el => chartRefs.current.percentageChart = el} />
        </div>
      )}
    </div>
  );
